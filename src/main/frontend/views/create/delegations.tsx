    ...
    <div className="flex flex-col h-full items-center justify-center p-l text-center box-border">
      ...
      <div className="flex flex-col gap-m w-full max-w-md">
        ...
        <TextField
          ...
          value={carQuantity.toString()} // Convertir a string para TextField
          onValueChanged={({ detail }) => setCarQuantity(parseInt(detail.value) || 0)}
          type="number"
        />
        <TextField
          label="Latitud"
          value={lat.toString()}
          onValueChanged={({ detail }) => setLat(parseFloat(detail.value) || 0)}
          type="number"
        />
        <TextField
          label="Longitud"
          value={longVal.toString()} // Corregido: 'longVal'
          onValueChanged={({ detail }) => setLongVal(parseFloat(detail.value) || 0)}
          type="number"
        />
        {/* El campo 'operation' se mantiene fijo como 'profile' o se puede hacer un Select si hay más tipos */}
        {/* <TextField
          label="Operación"
          value={operation}
          onValueChanged={({ detail }) => setOperation(detail.value)}
          placeholder="Ej: profile"
        /> */}
