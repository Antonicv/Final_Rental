...
import { useQuery } from '@tanstack/react-query'; // Asegúrate de tener esto importado
...
const getCarThumbnailImageUrl = (car: Car) => {
    const isCurrentCarVintage = car.year !== null && car.year < 2000; // Determina si el coche es vintage
...
export default function BookingsView() {
    ...
    const fetchAllData = async (userIdToFilter: string | null = null) => {
        ...
        try {
            ...
            // 2. Cargar todas las delegaciones (CORRECCIÓN: de getAllProfileDelegations a getAllDelegations)
            const delegationsResult = await DelegationEndpoint.getAllDelegations(); // <--- CAMBIO AQUÍ
            ...
            // 3. Cargar reservas (CORRECCIÓN: Usar getBookingsByUserId si hay filtro)
            let bookingsToDisplay: Booking[];
            if (userIdToFilter) {
                console.log(`DEBUG (BookingsView): Filtrando reservas por usuario: ${userIdToFilter}`);
                bookingsToDisplay = await UserEndpoint.getBookingsByUserId(userIdToFilter) ?? []; // <--- CAMBIO AQUÍ
            } else {
                console.log("DEBUG (BookingsView): Cargando todas las reservas.");
                bookingsToDisplay = await DelegationEndpoint.getAllBookings() ?? [];
            }
            const validBookings = Array.isArray(bookingsToDisplay) ? bookingsToDisplay : [];
            setAllBookings(validBookings); // Almacena todas las reservas (o las filtradas, según la llamada)
            setFilteredBookings(validBookings); // Las reservas filtradas son las que acabamos de obtener
    ...
    const handleDeleteBooking = async (booking: Booking) => {
        ...
        if (confirmDelete) {
            try {
                ...
                // Asumiendo que el deleteBooking en DelegationEndpoint.java requiere bookingId
                // Si requiere carId y startDate, adapta la llamada según tu DelegationEndpoint
                if (booking.bookingId) { // Preferible usar bookingId si existe y es único
                    await DelegationEndpoint.deleteBooking(booking.bookingId); 
                    setError(`Reserva ${booking.bookingId} borrada con éxito.`);
                } else { // Fallback si bookingId no está disponible en este contexto para el borrado
                    setError('No se pudo borrar la reserva: ID de reserva no disponible.');
                    return;
                }
                ...
                <div className="w-full max-w-2xl text-left">
                    <ul className="list-none p-0">
                        {filteredBookings.map(booking => { // Iterar sobre filteredBookings
                            ...
                            const isCurrentCarVintage = car ? (car.year !== null && car.year < 2000) : false; // Necesario para la conversión de precio
                            ...
                            if (car && booking.startDate && booking.endDate) {
                                try {
                                    ...
                                    if (duration > 0 && car.price !== undefined && car.price !== null) {
                                        const calculatedPrice = car.price * duration;
                                        totalPrice = isVintageMode && isCurrentCarVintage
                                            ? `${(calculatedPrice ).toLocaleString(undefined, { maximumFractionDigits: 0 })} Pts`
                                            : `${calculatedPrice.toFixed(2)} €`;
                                    } else {
                                        totalPrice = 'Fechas/Precio inválidos';
                                    }
                                } catch (e) {
                                                    ...
                                                    <p className="text-gray-700">
                                                        ...
                                                        <strong>
                                                            {isVintageMode && isCurrentCarVintage
                                                                ? `${(car.price !== undefined && car.price !== null ? car.price : 'N/A').toLocaleString(undefined, { maximumFractionDigits: 0 })} Pts`
                                                                : `${car.price || 'N/A'} €`}
                                                        </strong>
                                                    </p>
                                                    <p className="text-gray-700">
